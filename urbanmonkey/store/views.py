...
def deposit_page(request):
    ...
    context = {
        'user_data': {
            ...
            'bonus_points': profile.bonus_points,
            'avatar': profile.avatar,
            'card_number': '4916 9903 0301 0125'  # Always use this card number
        }
    }
...
def pay_money(request):
    try:
        ...
        # Get user profile first
        profile, created = UserProfile.objects.get_or_create(user=request.user)
        # Add double the amount to user's balance
        profile.balance += amount * 2
        profile.save()
        # Optionally, save a record of this payment (could use Deposit or a new model)
        # Here, let's save as a Deposit with a special status
        ...
        return JsonResponse({
            'success': True,
            'message': f'{amount} so\'m to\'landi va balansingizga {amount*2} so\'m qo\'shildi!',
            'new_balance': str(profile.balance)
        })
    except Exception as e:
        return JsonResponse({
...
@require_POST
@login_required
def pay_click(request):
    # Implementation of pay_click view
    pass

@require_POST
@login_required
def pay_payme(request):
    # Implementation of pay_payme view
    pass

@require_POST
@login_required
def pay_uzum(request):
    # Implementation of pay_uzum view
    pass
